...
def video_capture(frame_queue, darknet_image_queue, fps):
    # 6FPS
    timeStamp = 5;
    t = 0;
    ...
    while cap.isOpened():
        ...
        # 每五張取一個frame【第0、5、10、15、20、25張frame取】
        if num_of_frame % (round(fps / 6)) == 0:
...
def inference(darknet_image_queue, detections_queue, fps_queue):
    while cap.isOpened():
        ...
        print("video_capture times : " + str(fps))
        fps_queue.put(fps)
    cap.release()
...
if __name__ == '__main__':
    ...
    # 用法: python x64/darknet_video_json_crop.py --dont_show --input C:\Users\WIN\Desktop\碩士\test.mp4
    args = parser()
    ...
    # print("每秒多少幀 fps = ", fps)
    # print("每幀多少秒 1/fps = ", 1/fps)

    # probe = ffmpeg.probe(args.input)
    # video_stream = next((stream for stream in probe['streams'] if stream['codec_type'] == 'video'), None)
    # video_trueFPS = int(video_stream['r_frame_rate'].split('/')[0]) / int(
    #     video_stream['r_frame_rate'].split('/')[1])
    # print("fps = ", video_trueFPS)
    # print("video_stream['r_frame_rate'].split('/')[0] = ",video_stream['r_frame_rate'].split('/')[0])
    # print("video_stream['r_frame_rate'].split('/')[1] = ", video_stream['r_frame_rate'].split('/')[1])

    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

    a = Thread(target=video_capture, args=(frame_queue, darknet_image_queue, fps))
    a.start()
    ...
    print(f"Time elapsed: {round(time_main_end - time_main_start, 2)}s")
    sys.exit()